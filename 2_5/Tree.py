...
class TreeMixture:
    ...
    def prob_observation(self, observation):
        """Compute the probability of the observation by marginalising over all
        the possible trees.

        Parameters:
            observation -- Numpy array (num_nodes)
        ...
        for k in range(len(self.clusters)):
            p += self.prob_observation_given_tree(observation, k) * self.pi[k]
        return p


    def prob_observation_given_tree(self, observation, k):
        """Compute the probability of the observation given k-th tree

        Parameters:
            observation -- Numpy array (num_nodes)
            k           -- Index of the tree to use in computation

        Returns:
            Probability of the observation given that it was generated by k-th tree
        """
        tree = self.clusters[k]
        ...
        while len(visit_list) != 0:
            ...
            if cur_node.ancestor == None:
                ...
                parent_val = observation[int(cur_node.ancestor.name)]
                p *= cur_node.cat[parent_val][cur_node_val]
            visit_list = visit_list[1:] + cur_node.descendants
        return p

    def likelihood_dataset(self, samples):
        loglik = 0
        for sample in samples:
            prob = self.prob_observation(sample)
            loglik += np.log(prob)
        return loglik
    ...
    def sample_mixtures(self, num_samples, seed_val=None):
        ...
        sample_assignments = np.random.choice(np.arange(self.num_clusters),
                                              p=self.pi, size=self.num_samples)
    ...
    def print(self):
        ...
        print("\tnum_nodes: ", self.num_nodes, "\tnum_clusters: ", self.num_clusters,
              "\tpi: ", self.pi)
    ...
    def save_mixture(self, filename, save_arrays=False):
        """ This function saves the tree mixture in a pickle file.
            If save_arrays=True, the function saves some of the features
        ...
        if save_arrays:
            ...
            print("Saving pi to ", pi_filename, ", samples to: ", samples_filename,
...
class Node:
    """ Node Class
        Class for tree nodes. Each node has a name, a list of categorical distribution
        probabilities (thetas), an ancestor node and the list of children nodes.
    """
    ...
    def print(self):
        ...
        if self.ancestor is None:
            print("\tNode: ", str(self.name), "\tParent: ", self.ancestor,
                  "\tNum Children: ", len(self.descendants),
                  "\tCat: ", self.cat)
        else:
            print("\tNode: ", str(self.name), "\tParent: ", str(self.ancestor.name),
                  "\tNum Children: ", len(self.descendants),
                  "\tCat: ", self.cat)
...
class Tree:
    """ Tree Class
        Class for tree structures. Each tree has a root node, the number of nodes,
        the number of leaves, k (the number of possible values), alpha for dirichlet prior
        to categorical distributions, the number of samples, the list of samples
    ...
    def create_random_tree_fix_nodes(self, seed_val, k, num_nodes=10, max_branch=5, alpha=None):
        ...
        while cur_num_nodes != num_nodes:  # len(visit_list) != 0 and cur_num_nodes < num_nodes:
            ...
            if cur_node == self.root:
                num_children = np.random.randint(1, min(max_branch + 1,
                                                        num_nodes - cur_num_nodes + 1))
            else:
                num_children = np.random.randint(0, min(max_branch + 1,
                                                        num_nodes - cur_num_nodes + 1))
    ...
    def get_topology_array(self):
        """ This function returns the tree topology as a numpy array. Each item represent the
    ...
    def print(self):
        ...
        if self.num_leaves > 0:
            ...
            print("\tnum_nodes: ", self.num_nodes, "\tnum_leaves: ", self.num_leaves,
                  "\tk: ", self.k, "\tnum_samples: ", self.num_samples,
                  "\talpha: ", self.alpha, "\tNewick: ", self.newick)
    ...
    def save_tree(self, filename, save_arrays=False):
        ...
        if save_arrays:
            ...
            print("Saving topology to ", topology_filename, ", theta to: ",
                  theta_filename, ",  samples to ",
            ...
            print("Saving topology to ", topology_filename, ",  samples to ",
                  samples_filename, " and ",
